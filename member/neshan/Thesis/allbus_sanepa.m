...
%% === Loss for DVR at each bus using optimal rating ===
fixedP_MW = best(1);
loss_per_bus = zeros(1, n_buses);

for b = 2:n_buses
    test_chrom = [fixedP_MW, b];
    loss_per_bus(b) = compute_loss_kW(test_chrom, bij, bi0, b00, n_buses);
end

fprintf('\n=== Loss for DVR at each bus (P = %.3f MW) ===\n', fixedP_MW);
for b = 2:n_buses
    fprintf('Bus-%d: %.4f kW\n', bus_map(b), loss_per_bus(b));
end
